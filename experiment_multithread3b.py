...
logging.basicConfig(filename='experiment3b.log', filemode="a", format='%(asctime)s - %(levelname)s: %(message)s', level='DEBUG')
logging.info("--------------------------------------------------------------------------------")
...
def compute(clf_name, clf, metric_names, metrics, random_state, weights, drift):
    ...
    try:
        ...
        for i_metric, metric_name in enumerate(metric_names):
                ...
                filename = "results/experiment3b/metrics/gen/%s/%s/%s/%s.csv" % (drift, stream_name, metric_name, clf_name)
                if not os.path.exists("results/experiment3b/metrics/gen/%s/%s/%s/" % (drift, stream_name, metric_name)):
                    os.makedirs("results/experiment3b/metrics/gen/%s/%s/%s/" % (drift, stream_name, metric_name))
